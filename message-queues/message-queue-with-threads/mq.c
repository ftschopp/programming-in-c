...
int borrar_mensajes(int msqid)
{
	...
	do
	{
		res = msgrcv(msqid, (struct msgbuf *)&msg, sizeof(msg.from)+sizeof(msg.event)+sizeof(msg.info), 0, IPC_NOWAIT); 
	}while(res>0);
	return res;
}

int showMessage(Message *msg)
{
   printf("\nCabecera Message TO %d FROM %d EVENT TYPE %d", msg->to, msg->from, msg->event);
   printf("\nINFO Message Numero %d", msg->info.numero);
}